...
struct Button {
    ...
    void setup(const sf::Font& font, const std::string& text, sf::Vector2f pos, sf::Vector2f size,
    ...
    bool isClicked(const sf::Event& e) const {
        ...
        return shape.getGlobalBounds().contains(static_cast<float>(e.mouseButton.x),
...
struct InputField {
    ...
    void setup(const sf::Font& font, sf::Vector2f pos, sf::Vector2f size,
        ...
        displayText.setPosition(pos.x - size.x / 2.f + 14.f,
    ...
    void handleEvent(const sf::Event& e) {
        if (e.type == sf::Event::MouseButtonPressed) {
            focused = box.getGlobalBounds().contains(
                static_cast<float>(e.mouseButton.x), static_cast<float>(e.mouseButton.y));
            box.setOutlineColor(focused ? sf::Color{60, 160, 255, 220} : sf::Color{75, 135, 255, 160});
        ...
        if (e.type == sf::Event::TextEntered) {
            ...
            } else if (e.text.unicode >= 32 && e.text.unicode < 128 && value.size() < maxChars) {
...
namespace UI {
    ...
    // Fondo con degradado
    inline sf::VertexArray makeGradientBackground(unsigned W, unsigned H, sf::Color top, sf::Color bot) {
