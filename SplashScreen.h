...
public:
    ...
    void onEnter() {
        ...
        m_sub.setString("Programacion III");
    ...
    // update
    // Maneja el fade-in/hold/fade-out y devuelve MAIN_MENU cuando termina la animación
    GameScreen update(float dt) {
        ...
        sf::Uint8 alpha = 0;
        if (m_elapsed < 1.0f) {
            alpha = static_cast<sf::Uint8>(m_elapsed * 255.f);
        } else if (m_elapsed < 2.2f) {
            alpha = 255;
        } else if (m_elapsed < 3.0f) {
            alpha = static_cast<sf::Uint8>((1.f - (m_elapsed - 2.2f) / 0.8f) * 255.f);
        }
